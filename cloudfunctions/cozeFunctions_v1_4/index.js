...
exports.main = async (event, context) => {
  ...
  try {
    console.log('=== cozeFunctions_v1_4 调用开始 ===');
    ...
    if (finalWorkflowType === 'DRAW_CARD' && result.success) {
      try {
        ...
        console.log('[cozeFunctions_v1_4] 抽卡历史记录成功');
        
        // 记录成功后，获取更新后的抽卡配额信息
        try {
          drawCardQuotaInfo = await getDrawCardQuotaInfo(wxContext);
          console.log('[cozeFunctions_v1_4] 获取抽卡配额信息成功:', drawCardQuotaInfo);
        } catch (quotaError) {
          // 获取配额信息失败不影响主流程，只记录日志
          console.error('[cozeFunctions_v1_4] 获取抽卡配额信息失败（不影响使用）:', quotaError);
        }
      } catch (recordError) {
        // 记录失败不影响主流程，只记录日志
        console.error('[cozeFunctions_v1_4] 抽卡历史记录失败（不影响使用）:', recordError);
      }
    }
    ...
    console.log('=== cozeFunctions_v1_4 调用成功 ===');
    return response;
    
  } catch (error) {
    console.error('=== cozeFunctions_v1_4 调用失败 ===');
...
async function recordDrawHistory(wxContext, parameters, cozeResult) {
  ...
  try {
    ...
    const record = {
      ...
      interpretTime: now,
      drawDate: drawDate,
      cloudFunctionVersion: 'cozeFunctions_v1_4', // 记录云函数版本号
      isActive: true
    }
