 ...
 * 算法：AEAD_AES_256_GCM
 * 参考文档：https://pay.weixin.qq.com/docs/merchant/development/interface-rules/notification-rules.html
 *
 * @param {Object} resource - 回调中的 resource 对象
 * @param {string} apiKey   - 微信支付 APIv3 密钥（32字节）
 * @returns {Object} 解密后的订单数据
 */
function decryptPaymentResource(resource, apiKey) {
  const { ciphertext, nonce, associated_data } = resource;

  // APIv3 密钥作为 AES-256-GCM 的 key（32字节 UTF-8）
  const key = Buffer.from(apiKey, 'utf8');

  // Base64 解码密文；最后 16 字节是 GCM 认证标签
...
async function handlePaymentNotify(event) {
  try {
    ...
    console.log('[handlePaymentNotify] 收到支付回调', {
      ...
      resource_type: notifyData.resource_type,
      hasResource: !!notifyData.resource
    });

    // ── 解密微信支付 V3 回调数据 ──────────────────────────────────────────
    // 微信支付 V3 的回调 body 不是明文，而是 AES-256-GCM 加密的 resource 结构。
    // 需要先解密才能读取 out_trade_no、trade_state 等字段。
    let transactionData;
    if (notifyData.resource && notifyData.resource.ciphertext) {
      const apiKey = process.env.WECHAT_PAY_API_KEY;
      if (!apiKey) {
        console.error('[handlePaymentNotify] 未配置 WECHAT_PAY_API_KEY，无法解密回调数据');
        return {
          statusCode: 500,
          headers: addCorsHeaders({ 'Content-Type': 'application/json' }),
          body: JSON.stringify({ code: 'FAIL', message: '服务端配置错误' })
        };
      }
      try {
        transactionData = decryptPaymentResource(notifyData.resource, apiKey);
        console.log('[handlePaymentNotify] 解密成功', {
          out_trade_no:   transactionData.out_trade_no,
          trade_state:    transactionData.trade_state,
          transaction_id: transactionData.transaction_id
        });
      } catch (decryptErr) {
        console.error('[handlePaymentNotify] 解密失败', decryptErr.message);
        return {
          statusCode: 400,
          headers: addCorsHeaders({ 'Content-Type': 'application/json' }),
          body: JSON.stringify({ code: 'FAIL', message: '回调数据解密失败' })
        };
      }
    } else {
      // 兼容测试场景：body 直接是明文订单数据
      transactionData = notifyData;
      console.log('[handlePaymentNotify] 收到明文回调（测试模式）');
    }
    // ─────────────────────────────────────────────────────────────────────

    // 解析订单号
    const out_trade_no   = transactionData.out_trade_no;
    const transaction_id = transactionData.transaction_id;
    const trade_state    = transactionData.trade_state;

    if (!out_trade_no) {
      console.error('[handlePaymentNotify] 回调数据缺少订单号', transactionData);
      return {
        statusCode: 400,
        headers: addCorsHeaders({
          'Content-Type': 'application/json'
