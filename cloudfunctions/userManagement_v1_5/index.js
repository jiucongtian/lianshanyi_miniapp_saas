  ...
  try {
    switch (action) {
      ...
      case 'adminSearchUsers':
        return await adminSearchUsers(wxContext, data)
      case 'adminUpdateUserType':
        return await adminUpdateUserType(wxContext, data)
      default:
        return {
          success: false,
          error: '未知操作类型'
...
async function upgradeUserType(wxContext, data) {
  ...
  // 验证用户类型有效性
  const validUserTypes = ['guest', 'normal', 'student', 'premium', 'admin']
  if (!validUserTypes.includes(targetUserType)) {
    return {
      success: false,
      error: '无效的用户类型，支持的类型：guest, normal, student, premium, admin'
    }
  }
 ...
 */
async function checkAdminPermission(operatorOpenid) {
  const result = await db.collection('users').where({
    openid: operatorOpenid,
    isActive: true
  }).get()
  if (result.data.length === 0) return false
  const adminRole = result.data[0].adminRole || 'none'
 ...
 * 管理员查询用户（按手机号或昵称）
 */
async function adminSearchUsers(wxContext, data) {
  const { OPENID } = wxContext
  ...
  try {
    let query
    if (searchType === 'phone') {
      query = db.collection('users').where({ phoneNumber: keyword.trim(), isActive: true })
    } else {
      query = db.collection('users').where({ nickName: keyword.trim(), isActive: true })
    }
    ...
    console.error('adminSearchUsers 失败:', error)
 ...
 * 管理员更新用户类型
 */
async function adminUpdateUserType(wxContext, data) {
  const { OPENID } = wxContext
  const { targetUserId, newUserType } = data
  ...
  if (!validUserTypes.includes(newUserType)) {
    return { success: false, error: `无效的用户类型，支持：${validUserTypes.join(', ')}` }
  }
  ...
  try {
    ...
    const oldUserType = userResult.data.userType || 'guest'
    ...
    const targetConfig = await getUserTypeConfig(newUserType)

    return {
      success: true,
      message: `用户类型已更新为 ${targetConfig.displayName}`,
      data: {
        targetUserId,
        oldUserType,
        newUserType,
    ...
    console.error('adminUpdateUserType 失败:', error)
    return { success: false, error: '更新用户类型失败' }
  }
}
