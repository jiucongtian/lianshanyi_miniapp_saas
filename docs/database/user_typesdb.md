...
| dailyDrawQuota | number | 是 | - | 每日抽卡/智慧洞见配额(0=不可用，正整数=次数限制，-1=无限) ⚠️ 智慧洞见复用此字段 |
...
4. **配额管理**:
   - profileQuota表示档案创建配额
     - -1表示无限制
     - 其他数值表示具体配额数量
   - dailyDrawQuota表示每日抽卡/智慧洞见配额（⚠️ 智慧洞见复用此字段）
     - 0表示不可用（如临时用户）
     - 正整数表示每日次数限制
     - -1表示无限制
     - **注意**：抽卡功能和智慧洞见是同一个功能，共用此配额字段
...
### 临时用户 (guest)
- 档案配额：3个
- 抽卡/智慧洞见配额：0次（不可用，dailyDrawQuota=0）
- AI出报告配额：0次（不可用）
- 权限范围：
  - ✅ 查看小程序基础功能
  - ✅ 进行生辰八字计算
...
### 普通用户 (normal)
- 档案配额：50个
- 抽卡/智慧洞见配额：1次/天（dailyDrawQuota=1）
- AI出报告配额：1次/天
- 权限范围：
  - ✅ 临时用户的所有权限
  - ✅ 使用抽卡功能（每日3次）
...
### 高级用户 (premium)
- 档案配额：无限制
- 抽卡/智慧洞见配额：无限制（dailyDrawQuota=-1）
- AI出报告配额：无限制
- 权限范围：
  - ✅ 普通用户的所有权限
  - ✅ 无限抽卡次数
...
### 配额计算方式
1. **免费配额**: 在本表 (user_types) 中配置，按用户类型区分
   - **智慧洞见**: 使用 `dailyDrawQuota` 字段（复用现有字段）
   - **AI出报告**: 使用 `dailyAiReportQuota` 字段（新增字段）
...
### ⚠️ 重要说明：字段复用
- **智慧洞见配额**：复用现有的 `dailyDrawQuota` 字段，不需要新增字段
- **AI出报告配额**：新增 `dailyAiReportQuota` 字段
- 这样设计可以避免字段冗余，保持数据表结构简洁
