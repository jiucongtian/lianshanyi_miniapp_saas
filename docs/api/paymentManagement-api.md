...
**工作原理：**
1. 当调用 `queryOrderStatus` 查询订单时，如果订单状态为 `NOTPAY`，会自动调用微信支付查询接口
2. 如果微信支付返回的状态为 `SUCCESS`，会同步更新数据库中的订单状态
3. 同步成功后，检查 `grantInfo.status`：
   - 如果为 `pending`，自动调用 `handlePaymentSuccess` 发放配额
   - 如果为 `granted`，跳过（已发放）
   - 如果为 `failed`，跳过（需要人工处理）
4. 查询微信支付接口失败时，返回数据库中的订单状态，不影响查询结果

**客户端自动轮询：**
- 支付成功后，`FunctionController.purchaseFunction` 会自动轮询查询订单状态
- 轮询策略：延迟2秒后开始，每2秒查询一次，最多查询5次（共10秒）
- 如果查询到订单状态为 `SUCCESS`，立即停止轮询并刷新配额缓存
...
**重要特性：**
- ✅ **自动同步订单状态**：如果订单状态为 `NOTPAY`（未支付），接口会自动调用微信支付查询接口同步最新状态
- ✅ **自动发放配额**：如果同步到 `SUCCESS` 状态且配额未发放（`grantInfo.status === 'pending'`），会自动发放配额
- ✅ **幂等性保证**：通过检查 `grantInfo.status` 防止重复发放配额
- ✅ **容错处理**：查询微信支付接口失败时，返回数据库中的订单状态并记录错误日志
