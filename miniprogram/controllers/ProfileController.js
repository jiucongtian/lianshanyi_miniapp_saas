...
class ProfileController extends BaseController {
   ...
   */
  async initialize() {
    console.log('[ProfileController] 开始初始化页面');
    ...
    try {
      ...
      console.log('[ProfileController] 页面初始化完成');
    } catch (error) {
      console.error('[ProfileController] 页面初始化失败:', error);
   ...
   */
  async loadUserInfo() {
    console.log('[ProfileController] 开始加载用户信息');
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        console.log('[ProfileController] 用户信息加载成功:', {
          userType: this.userInfo.userType,
          profileQuota: this.userInfo.profileQuota,
          usedProfiles: this.userInfo.usedProfiles
        });
      } else {
        console.error('[ProfileController] 获取用户信息失败:', response.error);
        this._showError('获取用户信息失败：' + (response.error || '未知错误'));
      }
    } catch (error) {
      console.error('[ProfileController] 加载用户信息异常:', error);
  ...
  async loadProfiles(page = 1, limit = 20, isRefresh = false) {
    if (this.isLoading) {
      console.log('[ProfileController] 正在加载中，跳过重复请求');
      return;
    }
    
    console.log('[ProfileController] 开始加载档案列表，页码:', page, '每页数量:', limit);
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        console.log('[ProfileController] 档案列表加载成功，数量:', this.profileList.length);
      } else {
        console.error('[ProfileController] 获取档案列表失败:', response.error);
        this._showError('获取档案列表失败：' + (response.error || '未知错误'));
      }
    } catch (error) {
      console.error('[ProfileController] 加载档案列表异常:', error);
  ...
  async refreshProfiles() {
    console.log('[ProfileController] 开始刷新档案列表');
    ...
    try {
      ...
      console.error('[ProfileController] 刷新档案列表失败:', error);
      wx.stopPullDownRefresh();
  ...
  async loadMoreProfiles() {
    if (!this.hasMore || this.isLoading) {
      console.log('[ProfileController] 没有更多数据或正在加载中');
      return;
    }
    
    console.log('[ProfileController] 开始加载更多档案，页码:', this.currentPage + 1);
    
    try {
      await this.loadProfiles(this.currentPage + 1, this.pageSize, false);
    } catch (error) {
      console.error('[ProfileController] 加载更多档案失败:', error);
  ...
  async selectProfile(profileId) {
    console.log('[ProfileController] 选择档案:', profileId);
    ...
    try {
      ...
      if (!selectedProfile) {
        console.error('[ProfileController] 未找到档案数据:', profileId);
      ...
      console.log('[ProfileController] 档案选择成功:', selectedProfile.profileName);
      ...
      console.error('[ProfileController] 选择档案失败:', error);
  ...
  async deleteProfile(profileId) {
    console.log('[ProfileController] 开始删除档案:', profileId);
    ...
    try {
      ...
      if (!confirmed) {
        console.log('[ProfileController] 用户取消删除操作');
        return;
      }
      ...
      if (response.success) {
        console.log('[ProfileController] 档案删除成功:', profileId);
        ...
        console.error('[ProfileController] 删除档案失败:', response.error);
        this._showError('删除失败：' + (response.error || '未知错误'));
      }
    } catch (error) {
      console.error('[ProfileController] 删除档案异常:', error);
      this._hideLoading();
  ...
  showQuotaExceededDialog() {
    if (!this.userInfo) {
      console.warn('[ProfileController] 用户信息未加载，无法显示配额对话框');
      return;
    }
  ...
  addProfile() {
    ...
    console.log('[ProfileController] 跳转到添加档案页面');
  ...
  editProfile(profileId) {
    console.log('[ProfileController] 编辑档案:', profileId);
    ...
    try {
      ...
      if (!profile) {
        console.error('[ProfileController] 未找到要编辑的档案数据:', profileId);
        this._showError('档案数据异常');
        return;
      }
      ...
      console.log('[ProfileController] 档案数据已存储，准备跳转到编辑页面');
      
      this._navigateTo('/pages/addProfile/index', { mode: 'edit' });
      
    } catch (error) {
      console.error('[ProfileController] 编辑档案失败:', error);
  ...
  _handleSelectProfileEvent(data) {
    console.log('[ProfileController] 收到档案选中事件:', data);
  ...
  _handleProfileListRefreshEvent() {
    console.log('[ProfileController] 收到档案列表刷新事件');
  ...
  _handleProfileManagerReady() {
    console.log('[ProfileController] ProfileManager初始化完成');
  ...
  loadDataFromProfileManager() {
    console.log('[ProfileController] 从ProfileManager加载档案数据');
  ...
  _handleProfileSelection() {
    ...
    if (this.pendingSelectProfileId) {
      ...
      if (pendingProfile) {
        console.log('[ProfileController] 选中待选中的档案:', pendingProfile.profileName);
  ...
  _autoSelectFirstProfile() {
    ...
    console.log('[ProfileController] 自动选中第一个档案:', firstProfile._id, firstProfile.profileName);
  ...
  _clearCurrentSelection() {
    console.log('[ProfileController] 清除当前选中状态');
  ...
  _handleDeletedCurrentProfile() {
    if (this.profileList.length > 0) {
      // 如果还有其他档案，自动选中第一个
      this._autoSelectFirstProfile();
      console.log('[ProfileController] 删除当前选中档案后，自动选中第一个档案');
    } else {
      // 如果没有档案了，清除选中状态
      this._clearCurrentSelection();
      console.log('[ProfileController] 删除最后一个档案，清除选中状态');
    }
  }
  ...
  _handleUpgradeAction() {
    if (!this.userInfo) {
      console.warn('[ProfileController] 用户信息未加载，无法处理升级操作');
      return;
    }
   ...
   */
  onShow() {
    console.log('[ProfileController] 页面显示');
    ...
    if (profileManager.isReady()) {
      console.log('[ProfileController] ProfileManager已初始化，直接加载数据');
      this.loadDataFromProfileManager();
    } else {
      console.log('[ProfileController] ProfileManager未初始化，等待初始化完成');
    }
  }
   ...
   */
  onHide() {
    console.log('[ProfileController] 页面隐藏');
    super.onHide();
  }
   ...
   */
  onUnload() {
    console.log('[ProfileController] 页面卸载');
